...
public class ProcessVoteImplTest {
    ...
    void shouldCreateVoteWithProcessedStatus() {
        ...
        // Assert
        Mockito.verify(voteRepository).save(argThat(vote -> 
    ...
    void shouldCreateVoteWithUnprocessedStatusInitiallyThenSetToProcessed() {
        ...
        // Assert - O VoteFactory cria com status UNPROCESSED, mas depois é alterado para PROCESSED
        Mockito.verify(voteRepository).save(argThat(vote -> 
            vote.getStatus() == VoteStatus.PROCESSED
        ));
    }
    ...
    void shouldNotPublishEventWhenUserValidationFails() {
        ...
        // Assert - Verifica que o evento não foi publicado quando a validação do usuário falha
        Mockito.verify(eventPublisher, Mockito.never()).publish(any());
    }
    ...
    void shouldExecuteAllStepsInCorrectOrder() {
        ...
        // Verifica a ordem de execução: save vote -> find user -> publish event
        var inOrder = Mockito.inOrder(voteRepository, userRepository, eventPublisher);
